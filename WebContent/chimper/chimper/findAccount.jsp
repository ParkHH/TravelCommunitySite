...
$(function(){
	...
	$("button[name='findIdCancel']").click(function(){
		$($("form[name='findID']").find("input[name='member_name']")).val("");
		$($("form[name='findID']").find("input[name='email']")).val("");
	...
	$.ajax({
		...
		success:function(result){
			...
			if(json.id != "null"){
				...
				$("div[name='IdPrintZone']").html("");
				$("div[name='IdPrintZone']").append("<h2 style='color:red'>입력정보로 등록된 ID 가 존재하지 않습니다.</h2>");
			}
		}
	});
}
...
<div class="wrapper">
	...
	<div style="width:100%; height:80px; text-align:center;">
		<button type="button" name="goMain" onClick="history.back();" style=" width:200px; background-color:dodgerblue;">go Main</button>
	</div>
</div>
