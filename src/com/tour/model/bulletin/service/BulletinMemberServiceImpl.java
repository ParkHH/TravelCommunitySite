...
import com.tour.common.mail.MailSenderImpl;
...
public class BulletinMemberServiceImpl implements BulletinMemberService {
	...
	public int update(BulletinMember member) throws UpdateException{
		member.setPass(sequrityBean.textToHash(member.getPass()));
		System.out.println("BulletinMemberServiceImpl : update() : 변경될 비밀번호 : "+member.getPass());
	...
	//==================중복체크 관련(190519 : 정호진)=================
	@Override
	public BulletinMember check(BulletinMember member, int passKey) {
	      ...
	      if (member.getId() != null) {
	         ...
	         if(result==null) {
	        	 try {
					mailSenderImpl.mailSending(member, mailSender, passKey);
				} catch (Exception e) {
					e.printStackTrace();
				}
